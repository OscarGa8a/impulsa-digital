...
describe("content.ts", () => {
	...
	it("has same number of services in both locales", () => {
		expect(content.es.services.items.length).toBe(
			content.en.services.items.length,
		);
	});

	it("has same number of portfolio items in both locales", () => {
		expect(content.es.portfolio.items.length).toBe(
			content.en.portfolio.items.length,
		);
	});

	it("has same number of pricing plans in both locales", () => {
		expect(content.es.pricing.plans.length).toBe(
			content.en.pricing.plans.length,
		);
	});
